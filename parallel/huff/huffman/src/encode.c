...
int main(int argc, char* argv[]) {
	...
	// if(argc<2) { fprintf(stderr, "Usage: encode infile [-o outfile] [-s scheme_file] [-m message_file]\n"); return 1; }
	char* infile = "encodeIN.txt";
	char* schemefile = "/dev/null";
	char* messagefile = "/dev/null";
	// char* outfile = "/dev/null";
	char* outfile = "encodeOUT.txt";
	...
	*/
	...
	int num_bytes = fread(message, sizeof(char), 100000, fin);
	message[num_bytes] = '\0';
	...
	if(compress) {
		for(i=0;i<strlen(fullMessage);i+=8) {
			char buff[9] = { '\0' };
