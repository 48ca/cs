...
int main( int argc, char *argv[] ) {
	...
	if(located<2) {
		fprintf(stderr,ANSI_COLOR_RED "Usage: decode scheme_file message_file [-sc]\n");
	...
	char tree[100000] = { '\0' };
	register int flush;
	for(flush = 0;flush<100000;flush++) { tree[flush] = '\0'; } // Clear array
	char keyin[100000];
	char valin[100000];
	FILE* fkeys;
	FILE* fvals;
	...
	int key_size = fread(keyin, sizeof(char), 100000, fkeys);
	int val_size = fread(valin, sizeof(char), 100000, fvals);
