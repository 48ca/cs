...
void displayfunc()
{
   ...
   double x2 , y2 ;
   double x3 , y3 ;
   char stepstr[20];   glClear(GL_COLOR_BUFFER_BIT);   for( x = 0 ; x < N ; x++ )
   {
      for( y = 0 ; y < N ; y++ )
      { 
         if( t[y][x] == TREE ) 
   ...
   glColor3f( 0.0 , 0.0 , 0.0 ) ;   x1 = 0.825*N ; y1 = 0.875*N ;
   ...
   glVertex2f(x1,y1); glVertex2f(x2,y2); glVertex2f(x3,y3);
   glEnd();
   glColor3f( 1.0 , 1.0 , 1.0 ) ;   glRasterPos2f(0.85*N,0.9*N);
   conv( step , stepstr ) ;
   drawString(stepstr);
   glutSwapBuffers();}
void reshapefunc(int wscr,int hscr)
{
   ...
   glLoadIdentity();
   gluOrtho2D(0.0,1.0*N,0.0,1.0*N);   glMatrixMode(GL_MODELVIEW);
}
