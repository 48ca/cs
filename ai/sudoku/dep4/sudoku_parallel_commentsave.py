    ...
    if __name__ == "__main__":
        try:
            ...
            for index, result in enumerate(sorted(answers)):
                if answers[result] == None: raise RuntimeError("Invalid puzzle ({})".format(lowest+index+1))
            ...
            for i in validation_tasks:
                ...
                if not valid:
                    raise RuntimeError("Error validation: Uncaught invalid puzzle (the given puzzle was probably impossible to solve)")
                solved = fsolns[i]
                puzzle = fpuzzles[i+lowest]
                # print("{} {}".format(solved,puzzle))
                ...
                if i % 812 == 789: # So fast we don't need updates that often
