...
def genNeighbors(important, words):
    adj = set()
    for i in range(len(important)):
        pattern_string = important[:i]+"\w"+important[i+1:]+"(?=[ ])"
...
for index, word in enumerate(words):
    to_add = list(genNeighbors(word,word_string))
    for i,w in enumerate(to_add):
        if w not in words_dict: words_dict[w] = set()
        words_dict[w].update(set(to_add[:i] + to_add[i+1:]))
