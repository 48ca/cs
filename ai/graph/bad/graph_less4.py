...
def genWords(word,words):
    ret = []
    alphabet = "abcdefghijklmnopqrstuwxyz"
    for i in range(len(word)):
        for l in alphabet:
            tmp = word[:i] + l + word[i+1:]
            if tmp in words and word != tmp:
                ret.append(tmp)
    return ret
def allAdjacentTo(important,words):
    adj = set()
    words = genWords(important,words)
    adj.update(words)
    return adj
...
for index, word in enumerate(words):
    for i in allAdjacentTo(word,word_set):
        for j in words_dict[word]:
            if i!=j:
                print(word)
