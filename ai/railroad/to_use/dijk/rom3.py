...
def readNodes(graph,filename):
    ...
    for line in arr:
        ...
        print(n[0] + str(graph[n[0]]['coords']))
    f.close()
...
def astar(start, end, graph):
    ...
    while q:
        ...
        visited.add(v)
        c = curr['cost']
        path = cost_so_far[v][1] if v in cost_so_far and cost_so_far[v][0] <= c else curr['path']
        checked += 1
        ...
        for n in graph[v]["edges"]:
            ...
            # if graph[n]['gcost'] > c:
            new_cost = cost_so_far[v][0] + graph[v]['edges'][n]['weight']
            if n not in cost_so_far or new_cost < cost_so_far[n][0]:
                cost_so_far[n] = [new_cost,path]
