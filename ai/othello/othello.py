...
def scaledNumPossibleAfterMove(grid,id):
    pos = 10*(10-len(possiblemoves(grid,ids[id])))
    return pos
def isCorner(pos):
...
def bestMove(grid,possible,id,debug=True):
    ...
    for pos in possible.keys():
        scores[pos] = scaledNumPossibleAfterMove(mv(grid,pos,possible[pos],id),1-id)
        if(scores[pos] == 0): return pos
        scores[pos] += len(possible[pos]) * 100
