...
def min_play(board,id,level):
    ...
    for move in moves:
        ...
        score = max_play(clone,id,level+1)
        if(score < best_score):
            best_move = move
            best_score = score
    return best_score
...
def max_play(board,id,level):
    ...
    for move in moves:
        clone = mv(board,move,moves_dict[move],idl)
        score = min_play(clone,id,level+1)
        if(score > best_score):
            best_move = move
            best_score = score
    return best_score
...
def minimax(board,id,moves,moves_dict,scores,level=0):
    ...
    for move in moves:
        clone = mv(board,move,moves_dict[move],idl)
        score = min_play(clone,id,level+1)*100
        if score > best_score:
            best_move = move
            best_score = score + scores[move]
    return best_move
...
def bestMove(grid,possible,id,debug=True):
    ...
    for pos in possible.keys():
        ...
        scores[pos] += len(possible[pos]) * 1000
