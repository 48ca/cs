...
def min_play(board,id,level):
    ...
    if(level >= MAX_LEVEL):
        ...
        for x in corners:
            ...
            elif board[x] == ids[opid]:
                corner_score -= corner_dec*corner_weight
        edge_score = 0
        for x in edges:
            if board[x] == idl:
                edge_score += edge_weight
            elif board[x] == ids[opid]:
                edge_score -= edge_weight * edge_dec
        moves_score = moves_weight * len(list(moves_dict.keys()))
        return board.count(idl) - board.count(ids[opid]) + corner_score + moves_score # + edge_score
        # return board.count(ids[opid]) - board.count(idl)
    ...
    if not moves:
        ...
        for x in edges:
            if board[x] == idl:
                ...
                edge_score -= edge_weight * edge_dec
        return count_weight*(board.count(idl) - board.count(ids[opid])) + corner_score + edge_score
...
def minimax(board,id,moves,moves_dict,scores,level=0):
    ...
    best_score = float('-inf')
    for move in moves:
        # score = max_play(board,id,level+1)*700 + scores[move]
        clone = mv(board,move,moves_dict[move],idl)
        score = min_play(clone,1-id,level+1)*minimax_weight + scores[move]
        # score = max_play(clone,1-id,level+1)*700 + scores[move]
