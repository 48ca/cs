...
def main():
    ...
    def blur(img):
        ret = img
        for i in range(int(sys.argv[3].lower())):
            img = ret
            h = img.shape[0]
            w = img.shape[1]
            y = 1
    ...
    print("Greyscaling...")
    nimage = modes['e'](img.copy(),False)
    print("Detecting edges...")
    bimage = modes['b'](img.copy())
    print("Blurring...")
    ...
    cv2.imshow(title + desc['g'] + desc['b'], bgimage)
    cv2.imshow(title + desc['b'], bimage)
    cv2.imshow(title + desc['e'],nimage)
    cv2.imshow(title + desc['g'] + desc['e'], ngimage)
