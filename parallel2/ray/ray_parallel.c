...
triple e = { 0.20 , 0.60 , -1.00 } ; // the eye
double txttheta = 0.0;
...
void getColor(color* loc, triple trip, triple dir, int depth, double rot) {
	loc->r = BGR*AMBIENT;
	loc->g = BGG*AMBIENT;
	loc->b = BGB*AMBIENT;
	...
	if(min_shape > -1) {
		...
		if(min_shape > 1) c = mins->c;
			...
			if(min_shape) {
				...
				double dx = (rvp.x - s[min_shape]->x); // TOOK OUT /r;
				double dy = (rvp.y - s[min_shape]->y);
				double dz = (rvp.z - s[min_shape]->z);
				double rho = sqrt(square(dx) + square(dy) + square(dz));
				double theta = atan2(dz,dx)+M_PI;
				while(theta > twopi) { theta-=twopi; }
				theta += twopi * rot;
				double phi = acos(dy/rho);

				/*
				double ilon = (double)rot/textwidth;
				double lon = atan2(dy,dx) + M_PI/2; // acos(dz); // acos(dx/sqrt(dx*dx + dz*dz)); UGH
				// if(dz < 0) lon += M_PI;
				double lat = atan2(dz, sqrt(dx*dx+dy*dy)) + M_PI/2; // atan(dy/dx);
				*/

				/* old code */
				// double xang = dz > 0 ? acos(dx) : M_PI/2 - acos(dx);
				// double xr = textwidth / (M_PI);
				// int xc = (int)(xang * xr / sqrt(dx*dx + dz*dz)) + rot + textwidth;
				// int xc = (int)(dx * textwidth / sqrt(dx*dx + dz*dz) / 2.0 + rot + textwidth);
				// int yc = (int)((1 + dy) * textheight/2.0);
				/* end code */

				/* new code */
				// old: int xc = textwidth * dx/sqrt(dx*dx+dz*dz)/2.0 + 2*textwidth + rot;
				// int xc = (int)(textwidth/2.0 * (1+(dx/sqrt(dx*dx+dz*dz))) + rot);
				// int yc = (int)(textheight/2.0 * (1+dy));
				/* end code */

				/* tut code */
				// double txc = acos(dz) / M_PI;
				// int xc = textwidth * txc + rot;
				// int yc = textheight * acos(dx / sin(M_PI*txc)) / (2 * M_PI);
				/* end code */

				/* formula code */
				int xc = textwidth * theta / twopi;
				// double mercN = log(tan(M_PI/4 + theta/2));
				// int yc = textheight/2 - textwidth*mercN/(2*M_PI);
				int yc = textheight * phi / M_PI;
				/* end code */

				xc = xc % textwidth;
				if(yc >= textheight || yc < 0) {
					printf("Y1 ::: %f | %d\n",dy,yc);
					printf("Y2 ::: %s %d: %p\n",pname,rank,maxt);
					yc = yc % textheight;
				}
				int* tc = texture + textwidth * 3 * yc + 3*xc;
				// int* tc = texture;
				c->r = *(tc);
				c->g = *(tc+1);
				c->b = *(tc+2);
		...
		if(depth < REF_DEPTH_LIMIT) {
			...
			getColor(r,rvp,nvd,depth+1,rot);
